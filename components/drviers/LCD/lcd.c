...
//uint8_t lcd_buf[LCD_BUF_SIZE] = {0};
...
{   
	...
	while(t)
	{
		...
		MYDMA_Config1(DMA2_Stream5, DMA_Channel_3,(u32)&SPI1->DR,(u32)color1,num1);
		SPI_I2S_DMACmd(SPI1,SPI_I2S_DMAReq_Tx,ENABLE);
		MYDMA_Enable(DMA2_Stream5);
		while(1)
		{
			if(DMA_GetFlagStatus(DMA2_Stream5, DMA_FLAG_TCIF5)!=RESET)//等待通道4传输完成
			{
				DMA_ClearFlag(DMA2_Stream5, DMA_FLAG_TCIF5);//清除通道4传输完成标志
				break; 
			}
		}
	}
...
// 参数说明     color           color: 要填充的颜色数组首地址
// 返回参数     void
// 使用示例     lcd_color_fill(0, 0, 10, 10, &color);     
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void LCD_Color_Fill(u16 xsta, u16 ysta, u16 xend, u16 yend, u16 *color)
{
	...
	u16 heigh = yend - ysta + 1;
	u16 color1[256];
	//u8 color1[512];
	
	LCD_Address_Set(xsta+OFFSET_X,ysta+OFFSET_Y,xend+OFFSET_X,yend+OFFSET_Y);//设置显示范围
	LCD_CS_Clr();
	SPI1->CR1|=1<<11;//设置SPI16位传输模式
	SPI_Cmd(SPI1, ENABLE);//使能SPI
	...
	for(i = 0;i < heigh; i++){
		for(j = 0;j < width; j++){
//			color1[index++]=((*color)>>8) & 0xFF;
//			color1[index++]=(*color) & 0xFF;
//			color++;
		...
		index=0;
		MYDMA_Config1(DMA2_Stream5, DMA_Channel_3,(u32)&SPI1->DR,(u32)color1,width);
		SPI_I2S_DMACmd(SPI1,SPI_I2S_DMAReq_Tx,ENABLE);
		MYDMA_Enable(DMA2_Stream5);
		while(1)
		{
...
//	u16 color_buffer[width * height];
	
	LCD_Address_Set(xsta, ysta+OFFSET_Y, xend, yend+OFFSET_Y); //设置光标位置
...
void LCD_ShowPicture(u16 x,u16 y,u16 length,u16 width,const u8 pic[])
{
	...
	while(t)
	{
		...
		MYDMA_Config(DMA2_Stream5, DMA_Channel_3,(u32)&SPI1->DR,(u32)pic,num1);
		SPI_I2S_DMACmd(SPI1,SPI_I2S_DMAReq_Tx,ENABLE);
		MYDMA_Enable(DMA2_Stream5);
		while(1)
		{
