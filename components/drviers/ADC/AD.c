...
void bsp_InitAdc(void)
{
	...
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_0;/*通道3*/
	GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;/*不带上下拉*/
	GPIO_Init(GPIOC,&GPIO_InitStructure);/*初始化*/
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AIN;/*模拟输入*/
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_1;/*通道3*/
	GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;/*不带上下拉*/
	GPIO_Init(GPIOC,&GPIO_InitStructure);/*初始化*/
	/*通用控制寄存器的配置*/
	...
	ADC_RegularChannelConfig(ADC1,ADC_Channel_10,1,ADC_SampleTime_480Cycles);/*设置规则通道2 一个序列 采样时间 */
	ADC_RegularChannelConfig(ADC1,ADC_Channel_11,2,ADC_SampleTime_480Cycles);/*设置规则通道3 二个序列 采样时间 */
