...
static void OLED_GPIO_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd( RCC_AHB1Periph_GPIOB, ENABLE);	//打开GPIOB的外设时钟
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8 | GPIO_Pin_9;	//选择控制的引脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//普通输出模式
	GPIO_InitStructure.GPIO_OType = /*GPIO_OType_OD;*/GPIO_OType_OD; //推挽输出
...
{  
	unsigned char n;		    //定义变量
	
	OLED_WR_Byte (0xb0+x,OLED_CMD);    //从0~7页依次写入
	OLED_WR_Byte (0x00,OLED_CMD);      //列低地址
	OLED_WR_Byte (0x10,OLED_CMD);      //列高地址  
	for(n=start;n<end;n++)OLED_WR_Byte(0,OLED_DATA); //写入 0 清屏
