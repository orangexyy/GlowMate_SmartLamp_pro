...
#define TOUCH_OFFSET_X 0 //15
#define TOUCH_OFFSET_Y 0 //15
#define REVERSE 1
...
//	__HAL_RCC_GPIOB_CLK_ENABLE();
	...
	/* 初始化复位引脚 */
	GPIO_InitStructure.GPIO_Pin = TOUCH_RST_PIN;
	...
	GPIO_WriteBit(TOUCH_RST_PORT,TOUCH_RST_PIN,Bit_SET);
...
///判断是否被触摸
bool Touch_Screen_is_Presseed(void)
{
//	return CST816_IIC_ReadREG(FingerNum);
...
*********************************************************************************************************/
//获取触摸屏x坐标
uint16_t CST816_Get_X_AXIS(void)
{
	uint8_t	XL, XH;
	
	XL = CST816_IIC_ReadREG(XposL);
	XH = CST816_IIC_ReadREG(XposH);
	
	CST816_Instance.X_Pos = ((XH&0x0F)<<8) | XL + TOUCH_OFFSET_X;	//TOUCH_OFFSET_Y 为误差
	return CST816_Instance.X_Pos;
...
//获取触摸屏y坐标
uint16_t CST816_Get_Y_AXIS(void)
{
	uint8_t	YL, YH;

	
	YL = CST816_IIC_ReadREG(YposL);
	YH = CST816_IIC_ReadREG(YposH);
	
	CST816_Instance.Y_Pos = ((YH&0x0F)<<8) | YL +TOUCH_OFFSET_Y;	//TOUCH_OFFSET_Y 为误差
	
	return CST816_Instance.Y_Pos;
