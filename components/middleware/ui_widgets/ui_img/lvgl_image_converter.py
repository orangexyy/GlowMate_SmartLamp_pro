...
try:
    ...
    raise ImportError("Need pypng package, do `pip install pypng`")

# 改为可选导入 LZ4，不安装也能运行（仅 LZ4 压缩不可用）
try:
    import lz4.block
    LZ4_AVAILABLE = True
except ImportError:
    ...
    def convert(self, filename) -> bytes:
        ...
        try:
            ...
            raise BaseException(
                ...
                "`sudo apt install pngquant` (Linux) or "
                "`brew install pngquant` (macOS) or "
                "download from https://pngquant.org/ (Windows)")
...
def unpack_colors(data: bytes, cf: ColorFormat, w) -> List:
    ...
    if bpp == 8:
        ...
        if cf == ColorFormat.RGB565:
            ...
            for p in pixels:
                ret.append(bit_extend((p >> 11) & 0x1f, 5))
                ret.append(bit_extend((p >> 5) & 0x3f, 6))
                ret.append(bit_extend((p >> 0) & 0x1f, 5))
...
class LVGLImageHeader:
    ...
    def binary(self) -> bytearray:
        ...
        binary += uint16_t(self.flags)
        binary += uint16_t(self.w)
        binary += uint16_t(self.h)
        binary += uint16_t(self.stride)
        binary += uint16_t(0)
        return binary
    ...
    def from_binary(self, data: bytes):
        ...
        try:
            self.cf = ColorFormat(data[1] & 0x1f)
    ...
    def _compress(self, raw_data: bytes) -> bytearray:
        ...
        if self.compress == CompressMethod.RLE:
            ...
            if not LZ4_AVAILABLE:
                raise ValueError("LZ4 compression requires 'lz4' package. Install with `pip install lz4`")
...
class LVGLImage:
    ...
    def adjust_stride(self, stride: int = 0, align: int = 1):
        ...
        if align >= 1 and stride == 0:
            ...
            raise ParameterError(f"Invalid parameter, align:{align}, stride:{stride}")
        ...
        if stride < current.stride_default:
            raise ParameterError(f"Stride is too small:{stride}, minimal:{current.stride_default}")
        ...
        def change_stride(data: bytearray, h, current_stride, new_stride):
            ...
            if new_stride < current_stride:
                ...
                for i in range(h):
                    data_out.append(data_in[i * current_stride:(i + 1) * current_stride])
                    data_out.append(padding)
            return b''.join(data_out)

        palette_size = self.cf.ncolors * 4
        data_out = [self.data[:palette_size]]
        data_out.append(change_stride(self.data[palette_size:], self.h, current.stride, stride))

        if self.cf == ColorFormat.RGB565A8:
            a8_stride = self.stride // 2
            a8_map = self.data[-a8_stride * self.h:]
            data_out.append(change_stride(a8_map, self.h, current.stride // 2, stride // 2))
    ...
    def premultiply(self):
        ...
        if self.cf.is_indexed:
            ...
            palette = [multiply(palette[i], palette[i + 1], palette[i + 2], palette[i + 3]) for i in range(0, len(palette), 4)]
            ...
            for h in range(self.h):
                ...
                processed = b''.join([multiply(map[i], map[i + 1], map[i + 2], map[i + 3]) for i in range(0, line_width, 4)])
                self.data[offset:offset + line_width] = processed
        elif self.cf == ColorFormat.RGB565A8:
            def multiply(data, a):
                r = (data >> 11) & 0x1f
                g = (data >> 5) & 0x3f
                b = (data >> 0) & 0x1f
            ...
            for h in range(self.h):
                ...
                processed = b''.join([multiply((rgb[i + 1] << 8) | rgb[i], a[i // 2]) for i in range(0, line_width, 2)])
                self.data[offset:offset + line_width] = processed
        elif self.cf == ColorFormat.ARGB8565:
            def multiply(data, a):
                r = (data >> 11) & 0x1f
                g = (data >> 5) & 0x3f
                b = (data >> 0) & 0x1f
            ...
            for h in range(self.h):
                offset = h * self.stride
                map = self.data[offset:offset + self.stride]
                processed = b''.join([multiply((map[i + 1] << 8) | map[i], map[i + 2]) for i in range(0, line_width, 3)])
                self.data[offset:offset + line_width] = processed
        else:
    ...
    def set_data(self,
        ...
        if self.data_len != len(data):
            raise ParameterError(f"{self} data length error got: {len(data)}, expect: {self.data_len}, {self}")
    ...
    def from_data(self, data: bytes):
        ...
        return self.set_data(header.cf, header.w, header.h, data[len(header.binary):], header.stride)
    ...
    def to_bin(self,
        ...
        with open(filename, "wb+") as f:
            ...
            header = LVGLImageHeader(self.cf, self.w, self.h, self.stride, flags=flags)
            bin += header.binary
    ...
    def to_c_array(self,
        ...
        if compress != CompressMethod.NONE:
            ...
            data = self.data
        write_c_array_file(self.w, self.h, self.stride, self.cf, filename, outputname, self.premultiplied, compress, data)
    ...
    def to_png(self, filename: str):
        ...
        if self.cf.is_indexed:
            data = self.data
            palette = [(data[i * 4 + 2], data[i * 4 + 1], data[i * 4 + 0], data[i * 4 + 3]) for i in range(self.cf.ncolors)]
            data = data[self.cf.ncolors * 4:]
            encoder = png.Writer(self.w, self.h, palette=palette, bitdepth=self.cf.bpp)
            ...
            encoder = png.Writer(self.w, self.h, bitdepth=self.cf.bpp, greyscale=True, alpha=False)
            data = self.data
        elif self.cf.is_colormap:
            encoder = png.Writer(self.w, self.h, alpha=self.cf.has_alpha, greyscale=False)
    ...
    def from_png(self,
        ...
        if cf is None or cf.is_indexed:
    ...
    def _png_to_indexed(self, cf: ColorFormat, filename: str):
        ...
        if not 'palette' in metadata or not auto_cf and len(metadata['palette']) != 2 **cf.bpp:
            reader = png.Reader(bytes=PngQuant(256 if auto_cf else cf.ncolors).convert(filename))
            w, h, rows, _ = reader.read()

        palette = reader.palette(alpha="force")
        ...
        if palette_len != cf.ncolors:
            if not auto_cf:
                logging.warning(f"{path.basename(filename)} palette: {palette_len}, extended to: {cf.ncolors}")
    ...
    def _png_to_colormap(self, cf, filename: str):
        ...
        for y, row in enumerate(rows):
            ...
            for x, (r, g, b, a) in enumerate(zip(R, G, B, A)):
                ...
                if (self.rgb565_dither and cf in (ColorFormat.RGB565, ColorFormat.RGB565_SWAPPED, ColorFormat.RGB565A8, ColorFormat.ARGB8565)):
    ...
    def rle_compress(self, data: bytearray, blksize: int, threshold=16):
        ...
        while index < data_len:
            ...
            if repeat_cnt == 0:
                ...
                nonrepeat_cnt = self.get_nonrepeat_count(memview[index:], blksize, threshold)
                ctrl_byte = uint8_t(nonrepeat_cnt | 0x80)
                compressed_data.append(ctrl_byte)
                compressed_data.append(memview[index:index + nonrepeat_cnt * blksize])
...
class RAWImage():
    ...
    def to_c_array(self,
                   filename: str,
                   outputname: str = None):
        write_c_array_file(0, 0, 0, self.cf, filename, outputname, False, CompressMethod.NONE, self.data)
        ...
        for f in self.files:
            if self.cf in (ColorFormat.RAW, ColorFormat.RAW_ALPHA):
                ...
                if self.ofmt == OutputFormat.BIN_FILE:
                    img.to_bin(self._replace_ext(f, ".bin"), compress=self.compress)
                elif self.ofmt == OutputFormat.C_ARRAY:
                    img.to_c_array(self._replace_ext(f, ".c", outputname), compress=self.compress, outputname=outputname)
...
def main():
    ...
    parser.add_argument('--nemagfx', action='store_true',
                        help="export color palette for I8 images compatible with NEMA accelerator", default=False)
    ...
    parser.add_argument('--name',
                        default=None,
                        help="Specify name for output file (only for single file input)")
    ...
    ofmt = OutputFormat(args.ofmt) if cf not in (ColorFormat.RAW, ColorFormat.RAW_ALPHA) else OutputFormat.C_ARRAY
    compress = CompressMethod[args.compress]

    # 检查 LZ4 是否可用
    if compress == CompressMethod.LZ4 and not LZ4_AVAILABLE:
        raise ImportError("LZ4 compression requires 'lz4' package. Install with `pip install lz4` or use --compress NONE/RLE")
