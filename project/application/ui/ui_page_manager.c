...
#include "../ui_cont/ui_cont.h"  // 复用容器创建接口
#include <string.h>
                  ...
                  ui_page_on_exit_callback_func on_exit) 
{
    ...
    // 不在这里创建根容器，延迟到 ui_page_switch 时按需创建，节省内存
...
bool ui_page_switch(TE_UI_PAGE_ID target_page_id) 
{
    ...
    // 如果目标页面的根容器不存在，则创建它（延迟创建，节省内存）
    if (target_page_control->root == NULL) {
        ui_cont_create(&target_page_control->root, lv_scr_act(), LV_HOR_RES, LV_VER_RES);
        lv_obj_add_flag(target_page_control->root, LV_OBJ_FLAG_HIDDEN);  // 初始隐藏
        lv_obj_clear_flag(target_page_control->root, LV_OBJ_FLAG_SCROLLABLE);  // 禁止滚动
    }
