/****************************************************************************\
**  文件名称 :  upgrade_iap.c
**  功能描述 :  Bootloader IAP。反初始化外设、检查应用固件有效性、Xmodem 接收与写 Flash、跳转应用。
**  作    者 :  -
**  日    期 :  -
**  版    本 :  V0.0.1
\****************************************************************************/
...
void upgrade_iap_end(void)
{
    ...
    if (upgrade_check_app_valid(APP0_START_ADDRESS))
    {
        ...
        /* 若升级区从未写入（首次从 APP 无效进入 IAP），补写 boot_version，否则 APP 读不到 */
        upgrade_ensure_boot_version_if_needed();
...
int upgrade_load_a_block(uint32_t addr)
{
    ...
    if (upgrade_check_app_valid(addr) == 0)
    {
        ...
        printf("Entering IAP mode - please send firmware via Xmodem (e.g. SecureCRT/Xshell)\r\n");
        return 0;
    }
